...
export const createLesson = async (req, res) => {
  try {
    const { title, order, book, audioUrl } = req.body;

    /* ================= VALIDATION ================= */
    if (!title || !order || !book || !audioUrl) {
      return res.status(400).json({
        success: false,
        message: "Title, order, book and audio are required",
      });
    }
    ...
    if (existingLesson) {
      return res.status(400).json({
        success: false,
        message: `Lesson number ${order} hore ayuu uga jiraa book-kan`,
      });
    }
    ...
    const lesson = await Lesson.create({
      ...
      // IMPORTANT:
      // allows frontend to display lessons
      isActive: true,
    });
...
export const updateLesson = async (req, res) => {
  try {
    const { title, order, audioUrl } = req.body;
    ...
    /* ================= CHECK DUPLICATE IF ORDER CHANGED ================= */
